# EXP NO:7 4KB ROM Memory with Read and Write Operation using Verilog HDL and Testbench Verification
...
2. Design the Verilog Code for ROM:Write the Verilog code for a 4KB ROM memory with read and write capabilities.
3. Create the Testbench:Write a testbench to simulate both the read and write operations, verifying that the data is correctly written to and read from the memory
4. Add the Verilog Files:Add the ROM Verilog module and the testbench file to the project.
5. Run Simulation:Run the behavioral simulation in Vivado and check the memory's read and write operations.
6. Observe the Waveforms:Analyze the waveform to verify that the memory read and write operations work as expected.
7. Save and Document Results:Capture the waveform and include the simulation results in the final report.
...
![image](https://github.com/user-attachments/assets/abf85c2d-fa4e-4a57-86b3-f18a5e55d930)
